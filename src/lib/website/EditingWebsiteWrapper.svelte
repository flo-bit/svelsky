<script lang="ts">
	...
	import { BlueskyLogin, PopoverEmojiPicker } from '@fuxui/social';
	import { client, login } from '$lib/oauth/auth.svelte.js';
	import { Button, Heading, Input, Label, Modal, Navbar, ThemeToggle, toast, Toaster } from '@fuxui/base';
	...
	import { settingsModal } from './EditHead.svelte';
...
<Modal bind:open={settingsModal.show}>
	...
	<PopoverEmojiPicker
		onpicked={(emoji) => {
			settingsModal.favicon = emoji.unicode;
			settingsModal.edited = true;
		}}
	>
		{#snippet child({ props })}
			<Button size="iconLg" {...props} class="w-fit">
				{settingsModal.favicon || '👋'}
			</Button>
		{/snippet}
	</PopoverEmojiPicker>
</Modal>
...
<ThemeWrapper>
	...
	<Navbar class="mx-4 mt-3 max-w-3xl rounded-full px-4 md:mx-auto">
